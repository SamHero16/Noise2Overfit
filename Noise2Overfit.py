...
def train(train_path, save_location, nb_epochs):
    model = UNet()
    ...
    for epoch in range(nb_epochs):
        ...
        train_over_fitting_criteria = 0.0 # Keep track of overfit criteria
        ...
        for (x, y) in loader:
            ...
            # Calculate Overfitting Criteria - MSE(f(x), f(y)
            with torch.no_grad():
                model.eval()
                y_pred = model(y)
                overCrit = criterion(denoised, y_pred)
                train_over_fitting_criteria += overCrit.item()

        # Adjust for batching
        train_loss = train_loss / len(loader)
        train_over_fitting_criteria = train_over_fitting_criteria / len(loader)
        ...
        with torch.no_grad():
            ...
            # display model output on train images every 1000th epoch
        ...
        if epoch == nb_epochs - 1:
            overfit_images = (model(unique).detach().cpu().squeeze(1).numpy())
            for idx, image_array in enumerate(overfit_images):
                file_path = f"{save_location}/pseudoClean_{names[idx]}"
    ...
    # Log loss and criteria in wandb
        ...
        "Training Overfitting Criteria": train_over_fitting_criteria})


if __name__ == "__main__":
    wandb.init(project="Noise2Overfit", config={
            "batch size": "64" ,
            "dataset": '2023-08-30-FullDataSet',
            "epochs": 20000,
        }
    )

    train('2023-08-30-FullDataSet', '2023-08-30-PseudoClean', nb_epochs=20000)
    wandb.finish()
